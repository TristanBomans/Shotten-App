#!/usr/bin/env bun
/**
 * SUPABASE TO CONVEX PRODUCTION MIGRATION SCRIPT
 *
 * This script syncs data from Supabase to Convex PRODUCTION environment.
 * It reads all data from Supabase, converts it to Convex format,
 * and seeds the Convex PRODUCTION database.
 *
 ...
 * Or: CONVEX_PROD_URL=https://your-app.convex.cloud bun run migrate:prod
 */
...
//   CONVEX_PROD_URL=https://your-prod-app.convex.cloud
//
// Or pass env vars directly:
//   SUPABASE_URL=... SUPABASE_ANON_KEY=... CONVEX_PROD_URL=... bun run migrate:prod
// =============================================================================
...
async function migrateProd() {
  console.log("🚀 Starting Supabase to Convex PRODUCTION migration...\n");
  console.log("⚠️  WARNING: This will write to PRODUCTION environment!\n");
  ...
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    ...
    console.error(`  CONVEX_PROD_URL: ${CONVEX_PROD_URL ? '✓ set' : '✗ missing'}`);
    process.exit(1);
  }

  if (!CONVEX_PROD_URL) {
    console.error("❌ Missing Production Convex URL.");
    console.error("");
    console.error("Set CONVEX_PROD_URL in your .env.local file:");
    console.error("  CONVEX_PROD_URL=https://your-prod-app.convex.cloud");
    console.error("");
    console.error("Or pass it as an environment variable:");
    console.error("  CONVEX_PROD_URL=https://... bun run migrate:prod");
    process.exit(1);
  }

  console.log(`🎯 Target: ${CONVEX_PROD_URL}\n`);

  // Initialize clients
  console.log("📡 Connecting to Supabase...");
  const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

  console.log("📡 Connecting to Convex PRODUCTION...");
  const convex = new ConvexClient(CONVEX_PROD_URL);
  ...
  try {
    ...
    console.log(`\n📝 Note: Data synced to PRODUCTION environment.`);
    console.log(`   Supabase data is untouched and preserved.`);
